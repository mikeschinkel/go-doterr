...
func TestWithErr_Enrichment(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			parts:     []any{doterr.NewErr(ErrTestSentinel, "key1", "value1"), "key2", "value2"},
			checkMeta: true,
		...
		},
		{
			...
			parts:     []any{doterr.NewErr(ErrTestSentinel), "k1", "v1", "k2", "v2"},
			checkMeta: true,
...
func TestCombineErrs_Various(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "doterr entries",
			errs: []error{
				doterr.NewErr(ErrTestSentinel, "key", "value"),
				doterr.NewErr(ErrOtherSentinel, "key2", "value2"),
			},
			count: 2,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := doterr.CombineErrs(tt.errs)
...
func TestErrMeta_Extraction(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "simple entry",
			err:       doterr.NewErr(ErrTestSentinel, "key", "value"),
			expectKey: "key",
		},
		{
			name:      "joined error with entry first",
			err:       errors.Join(doterr.NewErr(ErrTestSentinel, "key", "value"), ErrCause),
			expectKey: "key",
		},
		{
			name:      "joined error with entry last",
			err:       errors.Join(ErrCause, doterr.NewErr(ErrTestSentinel, "key", "value")),
			expectKey: "key",
		},
		{
...
func TestErrValue_TypeSafety(t *testing.T) {
	...
	if val, ok := doterr.ErrValue[string](err, "string_key"); !ok || val != "string_value" {
		t.Errorf("string extraction failed: got %v, %v", val, ok)
	}

	// Test int extraction
	if val, ok := doterr.ErrValue[int](err, "int_key"); !ok || val != 42 {
		t.Errorf("int extraction failed: got %v, %v", val, ok)
	}

	// Test bool extraction
	if val, ok := doterr.ErrValue[bool](err, "bool_key"); !ok || val != true {
		t.Errorf("bool extraction failed: got %v, %v", val, ok)
	}

	// Test float extraction
	if val, ok := doterr.ErrValue[float64](err, "float_key"); !ok || val != 3.14 {
		t.Errorf("float extraction failed: got %v, %v", val, ok)
	}

	// Test wrong type
	if _, ok := doterr.ErrValue[int](err, "string_key"); ok {
		t.Error("expected type mismatch to return false")
	}

	// Test missing key
	if _, ok := doterr.ErrValue[string](err, "missing_key"); ok {
...
func TestErrors_Extraction(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "multiple sentinels",
			err:       doterr.NewErr(ErrTestSentinel, ErrOtherSentinel),
			expectLen: 2,
		},
		{
			name:      "with metadata",
		...
		{
			name: "joined error",
			err:  errors.Join(doterr.NewErr(ErrTestSentinel), ErrCause),
...
func TestFindErr_TypeAssertion(t *testing.T) {
	...
	err := doterr.NewErr(ErrTestSentinel, "request_id", "12345")
	if found, ok := doterr.FindErr[error](err); !ok {
	...
	joined := errors.Join(doterr.NewErr(ErrTestSentinel, "key", "value"), ErrCause)
	if _, ok := doterr.FindErr[error](joined); !ok {
		t.Error("expected to find error in joined error")
	}

	// Test finding in nested structure
	nested := doterr.NewErr(ErrTestSentinel, ErrOtherSentinel, "key", "value")
...
func TestAppendErr(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := doterr.AppendErr(tt.initial, tt.toAppend)
